...
var (
	...
	GOptsValidFilterSql []string = []string{"insert", "update", "delete", "ddl"}
...
func (this *ConfCmd) ParseCmdOptions() {
	...
	flag.StringVar(&sqlTypes, "sql", "", StrSliceToString(GOptsValidFilterSql, C_joinSepComma, C_validOptMsg)+". only parse these types of sql, comma seperated, valid types are: insert, update, delete,ddl; default is all(insert,update,delete)")
	...
	this.StatChan = make(chan BinEventStats, this.Threads*2)
	this.DdlEventChan = make(chan MyBinEvent, this.Threads*2)
...
func (this *ConfCmd) PrintBinlogFirsttime() {
