...
func main() {
	...
	if my.GConfCmd.WorkType != "stats" && len(my.GConfCmd.FilterSql) == 1 && my.GConfCmd.FilterSql[0] == "ddl" {
		wg.Add(1)
		go my.ProcessDdlEvent(my.GConfCmd, &wg)
	} else {
		if my.GConfCmd.WorkType != "stats" {
			wg.Add(1)
	...
	close(my.GConfCmd.DdlEventChan)
	wg.Wait()
}
