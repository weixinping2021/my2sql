...
func (b *BinlogSyncer) parseEvent(s *BinlogStreamer, data []byte) error {
	...
	case *RotateEvent:
		...
		if e.Header.Timestamp == 0 {
			b.cfg.Logger.Infof("start analyse %s", b.nextPos)
		} else {
			t := time.Unix(int64(e.Header.Timestamp), 0)
			// 格式化时间为 'YYYY-MM-DD HH:MM:SS' 格式
			formattedTime := t.Format("2006-01-02 15:04:05")
			b.cfg.Logger.Infof("finish analyse %s,binlog stop time is %s", b.nextPos, formattedTime)
		}
	case *FormatDescriptionEvent:
		t := time.Unix(int64(e.Header.Timestamp), 0)
		// 格式化时间为 'YYYY-MM-DD HH:MM:SS' 格式
		formattedTime := t.Format("2006-01-02 15:04:05")
		b.cfg.Logger.Infof("binlog start time is %s", formattedTime)
